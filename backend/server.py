...
async def get_archive_summary(current_user: dict = Depends(get_current_active_user)):
    """Arşiv özeti - Ay/Yıl bazında IZE sayıları"""
    ...
    result = await db.ize_cases.aggregate(pipeline).to_list(1000)
...
@api_router.get("/archive/{year}/{month}")
async def get_archive_by_month(year: int, month: int, current_user: dict = Depends(get_current_active_user)):
    """Belirli ay/yıldaki IZE case'lerini getirir"""
    # Admin tüm case'leri görebilir, user sadece kendisininkini
    if current_user['role'] == 'admin':
        query = {"year": year, "month": month}
    else:
        query = {"year": year, "month": month, "user_id": current_user['id']}
