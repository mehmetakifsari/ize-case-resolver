...
const AdminPricing = () => {
  ...
  const [formData, setFormData] = useState({ name: "", credits: 10, price: 100, currency: "TRY", is_popular: false, features: "", plan_type: "package", billing_period: "one_time" });
  ...
  const getApiErrorMessage = (error) => {
    ...
    return error?.response?.data?.message || error?.message || t("error");
  };
  ...
  const handleSubmit = async (e) => {
    ...
    setFormLoading(true);
    try {
      const normalizedBillingPeriod = formData.plan_type === "subscription"
        ? (formData.billing_period === "yearly" ? "yearly" : "monthly")
        : "one_time";
      const data = {
        ...formData,
  ...
  const openEdit = (plan) => {
    setEditPlan(plan);
    setFormData({ ...plan, billing_period: plan.billing_period || (plan.plan_type === "subscription" ? "monthly" : "one_time"), features: plan.features?.join("\n") || "" });
    setShowAdd(true);
  };

  const packagePlans = plans.filter((plan) => plan.plan_type !== "subscription");
  const subscriptionPlans = plans.filter((plan) => plan.plan_type === "subscription");
  ...
  return (
    <AdminLayout>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        ...
        <Dialog open={showAdd} onOpenChange={(open) => { setShowAdd(open); if (!open) { setEditPlan(null); setFormData({ name: "", credits: 10, price: 100, currency: "TRY", is_popular: false, features: "", plan_type: "package", billing_period: "one_time" }); } }}>
          ...
          <DialogContent className="sm:max-w-md">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label>{formData.plan_type === "subscription" ? "Dönem Kredisi / Period Credits" : t("planCredits")}</Label>
                  <Input type="number" min="1" value={formData.credits} onChange={(e) => setFormData({...formData, credits: parseInt(e.target.value) || 0})} required />
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div><Label>{t("planType")}</Label>
                  <Select value={formData.plan_type} onValueChange={(v) => setFormData({...formData, plan_type: v, billing_period: v === "subscription" ? "monthly" : "one_time"})}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
              ...
              {formData.plan_type === "subscription" && (
                <div>
                  <Label>{"Fatura Dönemi / Billing Period"}</Label>
                  <Select value={formData.billing_period} onValueChange={(v) => setFormData({...formData, billing_period: v})}>
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="yearly">{"Yıllık (Yearly)"}</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              )}
      ...
      {loading ? <div className="flex items-center justify-center h-64"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-primary"></div></div> : (
        <div className="space-y-8">
          <div>
            <h2 className="text-xl font-semibold mb-3">{t("package")}</h2>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {packagePlans.map(plan => (
                <Card key={plan.id} className={plan.is_popular ? "border-primary" : ""}>
                  <CardHeader className="pb-2">
                    ...
                    <CardDescription>{plan.credits} {t("credit")} - {t("package")}</CardDescription>
                  </CardHeader>
                  <CardContent>
              ...
              {packagePlans.length === 0 && <Card className="col-span-full"><CardContent className="p-8 text-center text-gray-500">Paket bulunamadı</CardContent></Card>}
            </div>
          </div>

          <div>
            <h2 className="text-xl font-semibold mb-3">{t("subscription")}</h2>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {subscriptionPlans.map(plan => (
                <Card key={plan.id} className={plan.is_popular ? "border-primary" : ""}>
                  <CardHeader className="pb-2">
                    ...
                    <CardDescription>{plan.credits} kredi - {getBillingPeriodLabel(plan.billing_period || "monthly")}</CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="text-3xl font-bold mb-4">{plan.price} {plan.currency === "TRY" ? "₺" : plan.currency === "USD" ? "$" : "€"}</div>
                    {plan.features && plan.features.length > 0 && (
              ...
              {subscriptionPlans.length === 0 && <Card className="col-span-full"><CardContent className="p-8 text-center text-gray-500">Abonelik planı bulunamadı</CardContent></Card>}
            </div>
          </div>

          {plans.length === 0 && <Card><CardContent className="p-8 text-center text-gray-500">{t("noPlans")}</CardContent></Card>}
        </div>
      )}
    </AdminLayout>
  );
